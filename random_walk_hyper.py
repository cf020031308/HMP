...
def get_first_order_part(nodes):
    ...
    for src in tqdm(nodes):
        ...
        # NOTE: We modify this line because there exists
        # uni-vertex hyperedges which raise errors here
        # with Coauth-AMiner, Stack-Biology, and Stack-Physiscs.
        # unnormalized_probs = ff_1st / np.sqrt(node_degree[dsts])
        unnormalized_probs = ff_1st if ff_1st.size == 0 else (
            ff_1st / np.sqrt(node_degree[dsts]))
